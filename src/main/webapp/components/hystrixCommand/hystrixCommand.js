...
(function(window) {
	...
	window.HystrixCommandMonitor = function(index, containerId, args) {
		...
		/* private */ function displayCircuit(data) {
			...
			//alert if error happened
			var _color = self.circuitErrorPercentageColorRange(data.errorPercentage);
			if(data.errorPercentage > 10 ){
			...
			$('#CIRCUIT_' + data.escapedName + ' a.errorPercentage').css('color', _color);
